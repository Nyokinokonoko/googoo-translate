...
// Style detection function (for detect mode)
export const detectTextStyle = async (
  inputText: string,
  llmConfig: Omit<LlmProviderConfig, "systemPrompt">
): Promise<string> => {
  if (!inputText.trim()) {
  ...
  const systemPrompt = `You are a text style analyzer. Analyze the given text and identify its style, tone, and characteristics. 
  Consider factors like:
  - Formality level (formal, casual, informal)
  - Tone (friendly, aggressive, neutral, kind, etc.)
  - Language complexity
  - Target audience
  - Cultural context
  
  Provide a brief analysis of the text's style and characteristics.`;
